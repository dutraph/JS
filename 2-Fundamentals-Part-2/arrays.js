...
const search = 'egg';
if (groceriesList.includes(search)) {
  //check if the element is inside of the array
...
if (groceriesList.indexOf(search) === -1) {
  ...
  if (groceriesList.length - groceriesList.indexOf(search) === 1) {
    console.log(
      `this is the last element, before it we have ${
        groceriesList[groceriesList.length - 2]
      }`
    );
  } else {
    console.log(
      `this is the ${groceriesList.indexOf(search)} element, it's between ${
        groceriesList[groceriesList.indexOf(search) - 1]
      } and ${groceriesList[groceriesList.indexOf(search) + 1]}.`
    );
  }
}
console.log(groceriesList);
